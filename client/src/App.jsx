...
function App() {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const [p, stats, favs] = await Promise.all([getProfile(), getDailyStats(), getFavorites()]);
            setProfile(p);
            setDailyStats(stats);
            setFavorites(favs);
        } catch (e) {
                ...
                {activeTab === 'dashboard' && (
                    <>
                        <div className="bg-slate-800 p-6 rounded-2xl shadow-lg border border-slate-700">
                            <h2 className="text-xl font-semibold mb-6 text-gray-200">Today's Summary</h2>
                                                    ...
                                                    <div className="space-y-3">
                                                        <div>
                                                            <input
                                                                ...
                                                                className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 transition-colors"
                                                            />
                                                        </div>
