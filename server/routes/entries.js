...
router.put('/:id', async (req, res) => {
    try {
        const { name, calories, date, weight } = req.body;
        const db = await getDb();
        await db.run(
            'UPDATE entries SET name = ?, calories = ?, date = ?, weight = ? WHERE id = ? AND user_id = ?',
            [name, calories, date, weight, req.params.id, req.user.id]
        );
        res.json({ success: true });
    } catch (error) {
        res.status(500).json({ error: error.message });
